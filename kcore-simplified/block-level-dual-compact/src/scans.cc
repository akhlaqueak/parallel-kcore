...
__device__ void compactBlock(unsigned int *degrees, unsigned int V, unsigned int* shBuffer,   unsigned int** glBufferPtr, unsigned int* bufTailPtr, unsigned int level){
    ...
    for(unsigned int base = 0; base < V; base += N_THREADS){
        ...
        unsigned int nv = addresses[BLK_DIM-1]+predicate[BLK_DIM-1];
        if(nv == 0) continue;
        
        if(THID == BLK_DIM - 1){  
            bTail = atomicAdd(bufTailPtr, nv);            
...
__device__ void compactWarp(unsigned int* temp, unsigned int* addresses, unsigned int* predicate, 
    ...
    // this warp don't have any thing to add in buffer... 
    // can save up to 10 ms 
    unsigned int nv = addresses[WARP_SIZE-1] + predicate[WARP_SIZE-1];
    if( nv == 0 ) return;
