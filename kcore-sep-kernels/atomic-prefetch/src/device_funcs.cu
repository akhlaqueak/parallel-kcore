...
__global__ void PKC(G_pointers d_p, unsigned int *global_count, int level, int V, 
    ...
    __shared__ unsigned int starts[BLK_NUMS];
    __shared__ unsigned int ends[BLK_NUMS];
    __shared__ volatile int prefetch;
    ...
    while(true){
        ...
        if(!prefetch && warp_id == 0 && base+THID < bufTail){
            v = readFromBuffer(shBuffer, glBuffer, base+THID);
            starts[THID] = d_p.neighbors_offset[v];
            ends[THID] = d_p.neighbors_offset[v+1];            
        }

        if(base == bufTail) break; // all the threads will break at same iteration
        i = base + warp_id;
